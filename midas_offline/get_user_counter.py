...
def build_user_counter(data):
  ...
  Sample = namedtuple('Sample', 'user_id, tag, package_name, pos_id, \
      ad_package_name, ad_id, ad_package_category, c_id, imp, click, install')
  for item in data[1]:
    ...
    for conf in user_count_feature_conf:
      ...
      user_counter[key] +=  Counter({'imp': sample.imp, 'click': sample.click, 'install': sample.install})
...
if __name__ == '__main__':
  ...
  rdd.saveAsTextFile('s3://sprs.push.us-east-1.prod/data/warehouse/midas_offline_model/user_counter/%s' % (date_str))
